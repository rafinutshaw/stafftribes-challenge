    ...
    return (
        <ul className="flex ">
            {values.map((item: ValueType, index) => {
                return <li key={`${item.value.toString()}${index}`}>
                    ...
                    <label htmlFor={`${item.value.toString()}${index}`} className={`${index == 0 ? "rounded-l-lg" : ''} ${index == values.length - 1 ? "rounded-r-lg" : ''} rounded-none p-2 text-gray-500 bg-white border border-gray-200 cursor-pointer peer-checked:border-blue-400 peer-checked:bg-blue-400 peer-checked:text-white hover:text-gray-600 hover:bg-gray-100 `}>
                        {item.label}
                    </label>
                </li>
            })}
